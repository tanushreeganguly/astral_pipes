...
if($title){
    $select = $objTypes->select("tbl_blog", "*", "is_delete = :is_delete AND blog_title LIKE :blog_title", $where, "blog_id DESC");
}else {
	$select = $objTypes->select("tbl_blog", "*", "is_delete = :is_delete", $where, "blog_id DESC");
}
#==== PAGINATION
...
if($title){
    $TypeArray = $objTypes->select("tbl_blog", "*", "is_delete = :is_delete AND blog_title LIKE :blog_title", $where, "blog_id DESC", $limit);
}else {
	$TypeArray = $objTypes->select("tbl_blog", "*", "is_delete = :is_delete", $where, "blog_id DESC", $limit);
}
?>
        ...
        <div class="box">
          ...
          <div class="box-body">
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  <td><input type="checkbox" class="case" id="DelCheckBox[]" name="DelCheckBox[]" value="<?=$val['blog_id']?>"/></td>
                  <td><?=$sino;?></td>
				   <td><?php
                  if($val['blog_image'] != ""){
                      ?>
                      <img src="../uploads/blog/<?php echo $val['blog_image']; ?>" width="75px" height="75px" />
                      <?php
                  }
                  ?></td>
                  <td><?=stripslashes($val['blog_title'])?></td>
                
                  <td><a href="act_blog.php?id=<?=$val['blog_id']?>&cat_type=<?=$cat_type?>&title=<?=$title?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_blog.php?id=<?=$val['blog_id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_blog.php?id=<?=$val['blog_id']?>&pgNo=<?=base64_encode($pgNo)?>&cat_type=<?=$cat_type?>&title=<?=$title?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
