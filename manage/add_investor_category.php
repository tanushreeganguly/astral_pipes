...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$res_arr	= $objTypes->fetchRow("SELECT * FROM tbl_investor_category WHERE id = :id", $params);
?>
<div class="content-wrapper">
  <!-- Content Header (Page header) -->
  <section class="content-header">
    <h1> <?=$mode?> investor Category <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_investor_category.php"><i class="fa  fa-navicon"></i> investor_category List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" method="post" action="act_investor_category.php" onsubmit="return validateForm();" enctype="multipart/form-data">
          <input type="hidden" name="id" id="id" value="<?=$res_arr['id']?>"  />
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            ...
            <div class="box-footer">
              ...
              <a href="list_investor_category.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
function validateForm(){
	if($("input#title").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("investor Category is Mandatory");
		$("input#title").focus();
		return false;
	}
	return true;
