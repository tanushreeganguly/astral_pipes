...
if($title){
	$select     = $objTypes->select("tbl_product_category", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC");
}
else{
	$select     = $objTypes->select("tbl_product_category", "*", "is_delete = :is_delete", $where, "title ASC");
}
...
$order		= 'title ASC';
if($title){
    $res_arr = $objTypes->select("tbl_product_category", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC", $limit);
}
else{
    $res_arr = $objTypes->select("tbl_product_category", "*", "is_delete = :is_delete", $where, "title ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="add_product_category.php"><i class="fa fa-plus-square"></i> Add product category</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_product_category.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_product_category.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_product_category.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_product_category.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_product_category.php';
			document.frmListing.submit();
		}
	}
	else
