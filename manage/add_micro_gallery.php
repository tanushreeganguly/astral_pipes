...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$res_arr	= $objTypes->fetchRow("SELECT * FROM tbl_micro_gallery WHERE id = :id", $params);
?>
<div class="content-wrapper">
  <!-- Content Header (Page header) -->
  <section class="content-header">
    <h1> <?=$mode?> Gallery <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_micro_gallery.php"><i class="fa  fa-navicon"></i> Gallery List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" method="post" action="act_micro_gallery.php"  enctype="multipart/form-data">
			 ...
			 <div class="form-group">
              ...
              <div class="form-group">
				...
				<div class="form-group">
					...
					<input type="file" class="form-control " id="image" name="image" value="" placeholder=" Image1" style="width:40%;" onchange="return Checkfile()">
					...
					<?php if($res_arr['image']){ ?>
						<a href="#" id='existing_image1'><img src="../uploads/micro_gallery/<?=stripslashes($res_arr['image'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/micro_gallery/<?=stripslashes($res_arr['image'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
					<?php } ?>						
					 <input type="hidden" name="store_image1" value="<?php echo $res_arr['image']; ?>">
				  </div>	
				  <div class="form-group">
					<label for="exampleInputEmail1">Thumbnail</label>
					<input type="file" class="form-control " id="thumbnail" name="thumbnail" value="" placeholder=" Image1" style="width:40%;" onchange="return Checkfile1()">
					<div class="alert alert-danger alert-dismissible" style="width:40%;margin-top:10px;">[Note:- File Size : 565 x 416  , MAX File Upload Size : 3MB]</div>
					<?php if($res_arr['thumbnail']){ ?>
						<a href="#" id='existing_image1'><img src="../uploads/micro_gallery/<?=stripslashes($res_arr['thumbnail'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/micro_gallery/<?=stripslashes($res_arr['thumbnail'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
					<?php } ?>						
					 <input type="hidden" name="store_image1" value="<?php echo $res_arr['thumbnail']; ?>">
				  </div>
            </div>
            <!-- /.box-body -->
            <div class="box-footer">
              <button type="submit" class="btn btn-primary" value="SAVE" name="SAVE" id="SAVE">Submit</button>
              <a href="list_micro_gallery.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
