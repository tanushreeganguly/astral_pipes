...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	$update     = $objTypes->update("tbl_applications", $params, "id = :id", $where);
	if($update){
		header("location:list_applications.php?sysmsg=1001&pgNo=".$pgNo);
		exit();
	}
    else{
		header("location:list_applications.php?sysmsg=4&pgNo=".$pgNo);
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	$where  = array(":id" => $id);
	$delete = $objTypes->update("tbl_applications", $params, "id = :id", $where);
	if($delete){
		header("location:list_applications.php?sysmsg=1002&pgNo=$pgNo");
		exit();
	}else{
		header("location:list_applications.php?sysmsg=4&pgNo=$pgNo");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_applications.php?sysmsg=8&pgNo=$pgNo");
		exit();
	}
	$Delete	= implode(',', $DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_applications SET is_active = '1' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_applications.php?sysmsg=1012&pgNo=$pgNo");
		exit();
	}
    else{
		header("location:list_applications.php?sysmsg=13&pgNo=$pgNo");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_applications.php?sysmsg=9&pgNo=$pgNo");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_applications SET is_active = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_applications.php?sysmsg=1013&pgNo=$pgNo");
		exit();
	}
    else{
		header("location:list_applications.php?sysmsg=14&pgNo=$pgNo");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_applications.php?sysmsg=10");
		exit();
	}
	$Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_applications SET is_delete = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_applications.php?sysmsg=1014");
		exit();
	}
    else{
		header("location:list_applications.php?sysmsg=4");
		exit();
	}
}
