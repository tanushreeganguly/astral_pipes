...
if($title){
	$select     = $objTypes->select("tbl_applications", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC");
}
else{
	$select     = $objTypes->select("tbl_applications", "*", "is_delete = :is_delete", $where, "title ASC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	for($i=0;$i<$total;$i++){   
		if(($POST["sortorder".$i]>=0)&&($POST['sortorder_id'.$i]>0)){
			...
			$update = $objTypes->update("tbl_applications", $params, "id = :id", $where);	
		}
	} 
	header("location:list_applications.php?sysmsg=1001");
	exit();
}

if($title){
    $res_arr = $objTypes->select("tbl_applications", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC", $limit);
}
else{
    $res_arr = $objTypes->select("tbl_applications", "*", "is_delete = :is_delete", $where, "title ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List applications<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_applications.php"><i class="fa fa-plus-square"></i> Add applications</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
				  ...
				  <td><?=stripslashes($val['short_description'])?></td><td align="center">                  
                  ...
                  <td>
                  <a href="add_applications.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_applications.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_applications.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_applications.php';
			document.frmListing.submit();
		}
	}
	else
