...
if($title){
	$select     = $objTypes->fetchAll("SELECT p.title as p_title,c.title as c_title, r.* FROM `tbl_product_range` r left join tbl_products_details p on r.product_id=p.id left join tbl_product_category c on c.id=r.product_cat_id where r.is_active=1 and r.is_delete=1 and r.product_name like '%".$title."%' order by r.id DESC");
}
else{
	$select     = $objTypes->fetchAll("SELECT p.title as p_title,c.title as c_title, r.* FROM `tbl_product_range` r left join tbl_products_details p on r.product_id=p.id left join tbl_product_category c on c.id=r.product_cat_id where r.is_active=1 and r.is_delete=1 ");
}
#==== PAGINATION
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List productrange<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_product_range.php"><i class="fa fa-plus-square"></i> Add productrange</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
                  <td><?=stripslashes($val['product_name'])?></td>				 
				  <td><?=stripslashes($val['p_title'])?></td>
				  <td><?=stripslashes($val['c_title'])?></td>
				  <td><?=stripslashes($val['product_subcat_name'])?></td>	
				  <td><?php
                  if($val['image'] != ""){ ?>
                      <img src="../uploads/product_range_images/<?php echo $val['image']; ?>" width="100px"  />
                      <?php } ?></td>
                  <td><a href="act_product_range.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
