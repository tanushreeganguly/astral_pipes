...
if($title){
	$select     = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 AND title LIKE  '%".$title."%' order by title ASC");
}else if($brand){ 
	$select     = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 $where order by title ASC");
}else{
	$select     = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 order by title ASC");
}
...
if($title){
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 AND title LIKE  '%".$title."%' order by title limit $limit");
}else if($brand){
	$res_arr     = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 $where order by title ASC limit $limit");
}else{
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_gallery where is_delete = 1 order by title ASC limit $limit");
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List gallery<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_micro_gallery.php"><i class="fa fa-plus-square"></i> Add gallery</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  if($res_arr){
			  	foreach($res_arr as $key=>$val){
						$brand_arr	= $objTypes->fetchRow("SELECT * FROM tbl_micro_brand WHERE is_active = 1 AND is_delete = 1 AND id=".$val['brand_id']);
			  ...
			  ?>
                <tr>
				  ...
				  <td><?=stripslashes($brand_arr['title'])?></td>
					<td><?php
                  if($val['image'] != ""){ ?>
                      <img src="../uploads/micro_gallery/<?php echo $val['image']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>				  
                  <td><a href="act_micro_gallery.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_micro_gallery.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_micro_gallery.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_gallery.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_gallery.php';
			document.frmListing.submit();
		}
	}
	else
