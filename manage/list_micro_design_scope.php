...
if($title){
	$select     = $objTypes->fetchAll("select * from tbl_micro_design_scope where is_delete = 1 AND title LIKE  '%".$title."%' order by title ASC");
}else{
	$select     = $objTypes->fetchAll("select * from tbl_micro_design_scope where is_delete = 1 order by title ASC");
}
...
$order		= 'title ASC';
if($title){
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_design_scope where is_delete = 1 AND title LIKE  '%".$title."%' order by title limit $limit");
}else{
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_design_scope where is_delete = 1 order by title ASC limit $limit");
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Designs scope<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_micro_design_scope.php"><i class="fa fa-plus-square"></i> Add Designs scope</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/micro_design_scope/<?php echo $val['image']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>				  
                  <td><a href="act_micro_design_scope.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_micro_design_scope.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_micro_design_scope.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_design_scope.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_design_scope.php';
			document.frmListing.submit();
		}
	}
	else
