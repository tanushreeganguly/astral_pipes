...
if($title){
	$select     = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 AND title LIKE  '%".$title."%' order by title ASC");
}else if($brand){ 
	$select     = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 $where order by title ASC");
}else{
	$select     = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 order by title ASC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	for($i=0;$i<$total;$i++){   
		if(($POST["sortorder".$i]>=0)&&($POST['sortorder_id'.$i]>0)){
			...
			$update = $objTypes->update("tbl_micro_combi_article", $params, "id = :id", $where);	
		}
	} 
	header("location:list_micro_combi_article.php?sysmsg=1001");
	exit();
}

if($title){
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 AND title LIKE   '%".$title."%' order by title limit $limit");
}else if($brand){
	$res_arr     = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 $where order by title ASC limit $limit");
}else{
    $res_arr = $objTypes->fetchAll("select * from tbl_micro_combi_article where is_delete = 1 order by title ASC limit $limit");
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="add_micro_combi_article.php"><i class="fa fa-plus-square"></i> Add Combi article</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_micro_combi_article.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_micro_combi_article.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_micro_combi_article.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_combi_article.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_micro_combi_article.php';
			document.frmListing.submit();
		}
	}
	else
