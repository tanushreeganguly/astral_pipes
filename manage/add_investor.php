...
if($id!=0){
	$res_arr	= $objTypes->fetchRow("SELECT * FROM tbl_investor_details WHERE id = :id", $params);
}
?>
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="list_investor.php"><i class="fa  fa-navicon"></i> client List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form id="clientForm" role="form" method="post" action="act_investor.php" onsubmit="return validateForm();" enctype="multipart/form-data">
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
			 ...
			 <div class="form-group">
			  ...
			  <select class="form-control" name="investor_id" id="investor_id" style="width: 40%" required>
				  <option value="">Select Category</option>
				  <?php
                      $cat_arr	= $objTypes->fetchAll("SELECT title, id FROM tbl_investor_category WHERE is_delete='1' AND is_active='1' ");
						if(sizeof($cat_arr) > 0){
							foreach($cat_arr as $cat_v){
								if($cat_v['id'] == $res_arr['investor_id']){
									$selected = 'selected';
								}
								else{
									$selected = '';
			  ...
			  <div class="form-group">
				...
				<a href="../uploads/investor_broucher/<?php echo $res_arr['broucher']; ?>" id='existing_image' target="_blank"><img src="../images/pdf-icon.jpg"  /></a>
				<?php } ?>
            ...
            <div class="box-footer">
              ...
              <a href="list_investor.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
jQuery(function(){
    jQuery('.img-wrap2 .close2').click(function() {
        ...
        if(confirm('Are you sure you want to delete selected images?')) {
           	window.location.href = '<?=base_url?>act_investor.php?id='+id+'&param=rimg&prodid=<?php echo $id ?>&pgNo=<?=$_REQUEST['pgNo']?>';
