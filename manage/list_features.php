...
$product	    =  ($POST['product']<>'') ? $POST['product'] : '' ;

if($POST['product']){
	$where =  " AND product_id = ".$POST['product'];
}

if($title){
	$title = '%'.$title.'%';
}
//echo "select * from tbl_features where is_delete = 1 $where order by title ASC"; 
if($title){
	$select     = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 AND title LIKE '$title' order by title ASC");
}else if($product){ 
	$select     = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 $where order by title ASC");
}else{
	$select     = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 order by title ASC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	for($i=0;$i<$total;$i++){   
		if(($POST["sortorder".$i]>=0)&&($POST['sortorder_id'.$i]>0)){
			...
			$update = $objTypes->update("tbl_features", $params, "id = :id", $where);	
		}
	} 
	header("location:list_features.php?sysmsg=1001");
	exit();
}

if($title){
    $res_arr = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 AND title LIKE '$title' order by title limit $limit");
}else if($product){
	$res_arr     = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 $where order by title ASC limit $limit");
}else{
    $res_arr = $objTypes->fetchAll("select * from tbl_features where is_delete = 1 order by title ASC limit $limit");
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List features<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_features.php"><i class="fa fa-plus-square"></i> Add features</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
                ...
                <select name="product" id="product" class="form-control" >
                	<option value="">Product</option>
                      <?php
						...
						$ProdArray	= $objTypes->fetchAll("SELECT * FROM tbl_products_details WHERE is_active = :is_active AND is_delete = :is_delete", $params);
						if(sizeof($ProdArray) > 0){
							foreach($ProdArray as $prod_v){
								if($prod_v['id'] == $product){
									$selected = 'selected';
								}
								else{
									$selected = '';
			  ...
			  if($res_arr){
			  	foreach($res_arr as $key=>$val){
						$prod_arr	= $objTypes->fetchRow("SELECT * FROM tbl_products_details WHERE is_active = 1 AND is_delete = 1 AND id=".$val['product_id']);
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_features.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_features.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_features.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_features.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_features.php';
			document.frmListing.submit();
		}
	}
	else
