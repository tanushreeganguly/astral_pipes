...
if($title){
	$select     = $objTypes->select("tbl_investor_relation", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC");
}
else{
	$select     = $objTypes->select("tbl_investor_relation", "*", "is_delete = :is_delete", $where, "id ASC");
}
...
$order		= 'title ASC';
if($title){
    $res_arr = $objTypes->select("tbl_investor_relation", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC", $limit);
}
else{
    $res_arr = $objTypes->select("tbl_investor_relation", "*", "is_delete = :is_delete", $where, "id ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Investor Relation<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_investor_relation.php"><i class="fa fa-plus-square"></i> Add Investor Relation</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              <thead>
				 ...
				 <tr>
                  ...
                  <th width="6%">#</th>
                  <!--th width="15%"> Text1</th>
				  <th width="15%"> Text2</th>
				  <th width="15%">Text3</th>-->
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
                  <!--td><?=substr(stripslashes($val['text1']),1,250)?></td>
				  <td><?=substr(stripslashes($val['text2']),1,250)?></td>
				  <td><?=substr(stripslashes($val['text3']),1,250)?></td-->
				  <td><a href="act_applications.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_investor_relation.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_investor_relation.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_investor_relation.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_investor_relation.php';
			document.frmListing.submit();
		}
	}
	else
