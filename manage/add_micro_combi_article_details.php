...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$res_arr	= $objTypes->fetchRow("SELECT * FROM tbl_micro_combi_article_details WHERE id = :id", $params);
?>
<div class="content-wrapper">
  <!-- Content Header (Page header) -->
  <section class="content-header">
    <h1> <?=$mode?> Combi article <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_micro_combi_article_details.php"><i class="fa  fa-navicon"></i> Combi article List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" method="post" action="act_micro_combi_article_details.php" onsubmit="return validateForm();" enctype="multipart/form-data">
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
			 <div class="form-group">
              ...
              <div class="form-group">
                <label for="exampleInputEmail1">Combi Article<?=MANDATORY?></label>
             ...
             <div class="form-group" id="product_type_div">
                <label for="exampleInputEmail1">Combi article</label>
                <select class="form-control" name="combi_article_id" id="combi_article_id_data" style="width: 40%">
                    <option value="">Select Combi article</option>
					<?php
                    $params     = array(":product_id" => $res_arr['product_id'],":is_active" => '1', ":is_delete" => '1');
                    $ProdArray	= $objTypes->fetchAll("SELECT title, id FROM tbl_micro_combi_article WHERE  is_active = :is_active AND is_delete = :is_delete AND product_id = :product_id ", $params);
			   ...
			   <div class="form-group">
				...
				<label for="exampleInputEmail1"> Image</label>
					...
					<a href="#" id='existing_image'><img src="../uploads/micro_combi_article/<?=stripslashes($res_arr['image'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/micro_combi_article/<?=stripslashes($res_arr['image'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
						<?php } ?>
				</div>
            ...
            <div class="box-footer">
              ...
              <a href="list_micro_combi_article_details.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
				...
				if(product_id){
					$.ajax({
						...
						success:function(response){							
							$("#combi_article_id_data").html(response);			
						}
					});
				}
		});
