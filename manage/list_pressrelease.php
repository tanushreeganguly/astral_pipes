...
$order		= 'title ASC';
if($title){
    $res_arr = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete AND title LIKE :title", $where, "id DESC", $limit);
}else{  
	$res_arr = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}
?>
			  ...
			  ?>
                <tr>
				  ...
				  <!--td><?=stripslashes($val['short_description'])?></td-->
                   <td><?php echo substr(stripslashes($val['description']),0,350);?>.....</td>
