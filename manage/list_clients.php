...
if($client){
	$select     = $objTypes->select("tbl_clients", "*", "is_delete = :is_delete AND client LIKE :client", $where, "client ASC");
}
else{
	$select     = $objTypes->select("tbl_clients", "*", "is_delete = :is_delete", $where, "client ASC");
}
...
$order		= 'client ASC';
if($client){
    $res_arr = $objTypes->select("tbl_clients", "*", "is_delete = :is_delete AND client LIKE :client", $where, "client ASC", $limit);
}
else{
    $res_arr = $objTypes->select("tbl_clients", "*", "is_delete = :is_delete", $where, "client ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List clients<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_clients.php"><i class="fa fa-plus-square"></i> Add clients</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
				  ...
				  <tr>
					<td colspan="3">
					 <div class="input-group">
					  <input type="text" name="client" placeholder="client..." class="form-control" value="<?=$client?>">
			  ...
			  if($res_arr){
			  	foreach($res_arr as $key=>$val){
					 $cat_arr	= $objTypes->fetchAll("SELECT title, id FROM tbl_client_category WHERE is_delete='1' AND is_active='1' AND id=".$val['client_cat_id']);
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
                  <td><?=stripslashes($val['client'])?></td>				 
				  <td><?=stripslashes($cat_arr[0]['title'])?></td>				  
                  <td><a href="act_clients.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_clients.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_clients.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_clients.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_clients.php';
			document.frmListing.submit();
		}
	}
	else
